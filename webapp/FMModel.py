...
def load_and_prep_datasets():
    ...
    #load data 
    ratingsdf = pd.read_csv(PATH + '/ratingsprocessedpipeline.csv', low_memory=False)

    #load lookups
    agelookupdf = pd.read_csv(PATH + '/agelookup.csv', low_memory=False)
    racelookupdf = pd.read_csv(PATH + '/racelookup.csv', low_memory=False)
...
def load_model(lenmodel, lenfeatures, device):
    ...
    modelinfer.load_state_dict(torch.load(PATH +'/fmstatedictRV2',map_location=torch.device('cpu')))
